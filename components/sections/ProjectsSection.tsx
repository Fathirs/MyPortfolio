...
// ── Image carousel dots ───────────────────────────────────────────────────────
function CarouselDots({ count, active, dotWidth = 6, activeWidth = 20 }: { count: number; active: number; dotWidth?: number; activeWidth?: number }) {
  if (count <= 1) return null;
  return (
    <div className="absolute bottom-[12px] left-1/2 -translate-x-1/2 flex gap-[6px] z-10">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="rounded-full transition-all duration-300" style={{
          width: i === active ? activeWidth : dotWidth, height: "6px",
...
function DesktopProjectsSection({ projects }: { projects: Project[] }) {
  ...
  return (
    <section className="dot-pattern w-full border-b border-dashed hidden lg:block" style={{ borderColor: "#d9d9d9" }}>
      <div className="max-w-[1440px] mx-auto px-[120px] py-[80px] flex flex-col gap-[40px] items-center">
        ...
        <ScrollReveal once threshold={0.15} className="flex flex-col gap-[24px] items-center text-center">
          ...
          <h2 className="text-[40px] font-medium leading-[1.2] text-[#0e0e16] max-w-[572px]" style={{ letterSpacing: "-1.2px" }}>
            Projects That Prove It Works.{" "}<br />Yours Could Be Next.
          </h2>
        </ScrollReveal>

        {/* Category filter bar */}
        <ScrollReveal once threshold={0.1} delay={60} className="flex">
          <div className="flex items-center gap-[7px] p-[4px] rounded-[16px] border" style={{ background: "#ebebeb", borderColor: "#e0e0e0" }}>
            {categories.map((cat) => (
              <CategoryBtn key={cat} cat={cat} isActive={cat === activeCategory} onClick={() => setActiveCategory(cat)} />
            ))}
          </div>
        </ScrollReveal>
  ...
  return (
    <Link
      ...
      style={{
        ...
        transition: `border-color 0.3s ease, box-shadow 0.3s ease, transform 0.4s ${SPRING}`,
    ...
    >
      ...
      <div className="flex flex-col gap-[16px] p-[32px] flex-1 min-w-0 self-stretch">
        <div className="flex flex-col gap-[16px] flex-1">
          <p className="font-mono text-[20px] leading-[1.5] text-[#0e0e16]">{project.num} {project.name}</p>
          <p className="text-[16px] leading-[1.5] text-[#606060]">{project.description}</p>
        </div>
        <div className="flex flex-wrap gap-[12px]">
          {project.tags.map((tag) => (
            <span key={tag} className="px-[16px] py-[8px] rounded-[100px] text-[16px] leading-[1.5] text-[#0e0e16]" style={{ background: "#f0f0f0" }}>{tag}</span>
          ))}
        </div>
      </div>
      ...
      <div className="relative shrink-0 rounded-[16px] overflow-hidden w-[450px] h-[350px]" style={{ background: "#f0f0f0" }}>
        <Image
          src={project.images[activeImg]} alt={project.name} fill className="object-cover object-top"
          style={{ transform: hovered ? "scale(1.03)" : "scale(1)", transition: "transform 500ms cubic-bezier(0.16,1,0.3,1)" }}
        />
      ...
      <div className="px-[24px] flex flex-col gap-[40px]">
        <div className="flex flex-col gap-[24px] items-center text-center">
          ...
          <h2 className="text-[28px] font-normal leading-[1.2] text-[#0e0e16] text-center">
            Projects that prove it works.{" "}yours could be next
          </h2>
        </div>
  ...
  return (
    ...
    >
      <div className="relative w-full rounded-[16px] overflow-hidden" style={{ height: "242px", background: "#f0f0f0" }}>
        <Image src={project.images[activeImg]} alt={project.name} fill className="object-cover object-top" />
        <CarouselDots count={project.images.length} active={activeImg} dotWidth={6} activeWidth={18} />
      </div>
      <div className="flex flex-col gap-[24px] px-[4px]">
        <div className="flex flex-col gap-[16px]">
          <p className="font-mono text-[20px] leading-[1.5] text-[#0e0e16]">{project.num} {project.name}</p>
          <p className="text-[14px] leading-[1.5] text-[#606060]">{project.description}</p>
        </div>
        <div className="flex flex-wrap gap-[8px]">
          {project.tags.map((tag) => (
            <span key={tag} className="px-[12px] py-[8px] rounded-[100px] text-[12px] leading-[1.5] text-[#0e0e16]" style={{ background: "#f0f0f0" }}>{tag}</span>
          ))}
        </div>
      </div>
    </Link>
